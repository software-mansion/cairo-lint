...
use cairo_lang_semantic::{
    ...
    helper::ModuleHelper,
    items::{free_function::FreeFunctionSemantic, imp::ImplSemantic, trt::TraitSemantic},
...
fn get_function_body_lowering<'db>(
    ...
    // `ConcreteFunctionWithBodyId::from_generic` only changes the semantic representation from generic to
    // concrete and does not actually check if the function is concrete.
    // The check above ensures that the obtained concrete function is lowerable.
...
/// Checks whether the function has any generic parameters in scope - either its own or from the enclosing impl or trait.
fn has_generic_params<'db>(
    db: &'db dyn Database,
    function_with_body_id: FunctionWithBodyId<'db>,
) -> bool {
    let is_empty = |params: cairo_lang_diagnostics::Maybe<&[semantic::GenericParam<'db>]>| {
        params.is_ok_and(|params| params.is_empty())
    };

    match function_with_body_id {
        FunctionWithBodyId::Free(free_function_id) => {
            !is_empty(db.free_function_generic_params(free_function_id))
        }
        FunctionWithBodyId::Impl(impl_function_id) => {
            !is_empty(db.impl_function_generic_params(impl_function_id))
                || !is_empty(db.impl_def_generic_params(impl_function_id.impl_def_id(db)))
        }
        FunctionWithBodyId::Trait(trait_function_id) => {
            !is_empty(db.trait_function_generic_params(trait_function_id))
                || !is_empty(db.trait_generic_params(trait_function_id.trait_id(db)))
        }
    }
}
