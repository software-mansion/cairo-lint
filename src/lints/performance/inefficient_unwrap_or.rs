...
fn get_all_unwrap_or_calls<'db>(
    ...
    let option_trait = ModuleHelper::core(db)
        .submodule("option")
        .trait_id("OptionTrait")
        .long(db);

    let result_trait = ModuleHelper::core(db)
        .submodule("result")
...
fn contains_nontrivial_expression<'db>(
    ...
    match argument_semantic {
        ...
        Expr::LogicalOperator(expr_logical_operator) => {
            ...
            let trivial = is_bool_literal(db, lhs_semantic) && is_bool_literal(db, rhs_semantic);
            !trivial
        }

        // Acces via Deref can be arbitrarily complex, thus it's non-trivial
...
fn fix_inefficient_unwrap_or<'db>(
    ...
    let suggestion = unwrap_or_call_on_object.as_syntax_node().get_text(db);

    let start_offset = unwrap_or_call_on_object.as_syntax_node().offset(db);
    let function_call_relative_offset = unwrap_or_call.as_syntax_node().offset(db) - start_offset;
    let left_parenthesis_end_offset = unwrap_or_call
        .arguments(db)
        .lparen(db)
        .as_syntax_node()
        .span(db)
