...
use cairo_lang_filesystem::db::{
    ...
    set_crate_config_for_input, set_generated_file_content_for_input, snapshot_crate_configs,
...
impl FixerDatabase {
    pub fn new_from(db: &dyn Database) -> Self {
        ...
        for (file_input, content) in snapshot_file_contents(db) {
            if content.is_some() {
                set_generated_file_content_for_input(&mut new_db, file_input, content);
            }
        }
